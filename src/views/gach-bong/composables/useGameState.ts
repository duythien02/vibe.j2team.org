...
export function useGameState(engine: Ref<GachBongModule | null>) {
    ...
    function backToMenu() {
        selectedTile = null
        state.value = { ...state.value, status: 'menu', selected: null, matchPath: null, hintTiles: null }
    }

    return { state, startGame, selectTile, requestHint, requestShuffle, tick, backToMenu, clearMatchAnimation }
}
